import { TeamCard } from "./TeamCard"
const teams = [
    {
        title: "President – Rising India School Excellence Management Private Limited",
        name: "Ms. Payal Gaba",
        img: "/images/leadership_team/payal.png",
        summary:
            "With over 25 years of leadership experience across education, strategy, and innovation...",
        content: (
            <>
                <p>
                    <strong className="text-[#831719]">Ms. Payal Gaba</strong> is a visionary Edupreneur redefining how children learn, think, and grow in the 21st century. As the{" "}
                    <strong className="text-[#831719]">Founder & CEO of BeyondSkool</strong>, India’s leading K-12 upskilling platform, she has pioneered experiential and skills-based education that has impacted over a million students across schools nationwide.
                </p>
                <p>
                    A graduate in Business Management and an alumni of{" "}
                    <strong className="text-[#831719]">MDI Gurgaon</strong>, Ms. Gaba blends corporate acumen with a deep passion for transformative learning. Her mission is to make{" "}
                    <strong className="text-[#831719]">world-class international schooling accessible in the rising cities of India</strong>, not as a privilege, but as a possibility.
                </p>
                <p>
                    Under her leadership,{" "}
                    <strong className="text-[#831719]">Rising India School of Excellence – The School of Tomorrow</strong> represents this vision in its truest form combining global pedagogy, Apple-enabled innovation, and Indian values to nurture confident, compassionate global citizens.
                </p>
                <p>
                    Honoured among{" "}
                    <strong className="text-[#831719]">India’s Top Women Entrepreneurs in Education</strong>, Ms. Gaba continues to be a driving force in reimagining schooling for the age of AI, creativity, and purpose-driven impact.
                </p>
            </>
        ),
        quote:
            "The role of education is not just to provide knowledge, but to build skills, character, and mindsets. Strong character and positive mindsets go a long way in nurturing leaders who not only shape themselves but also shape the world’s future.",
    },
    {
        title: "Founding Principal",
        name: "Aniket A. Salunkhe",
        img: "/images/leadership_team/aniket.jpg", 
        summary:
            "An experienced education leader with over 20 years of dedication to school education and student development across IB, Cambridge, CBSE, and ICSE curricula.",
        content: (
            <>
                <p>
                    Mr. Aniket A. Salunkhe is a passionate and experienced education leader with over twenty years of dedication to school education and student development. Born and educated in Mumbai, he holds degrees in Physics and Education along with a Master’s in Education, and began his journey as an IB Physics and Mathematics teacher. His classroom experience continues to shape his leadership approach, keeping student learning and wellbeing at the heart of every decision he makes.
                </p>

                <p>
                    Over the years, Mr. Salunkhe has served as Principal, Cluster Principal, and Chief Operations Officer, working across IB, Cambridge IGCSE, CBSE, and ICSE curricula. He brings extensive experience in building nurturing school environments, supporting teachers, strengthening academic quality, and partnering closely with parents to ensure every child feels supported, confident, and inspired to learn.
                </p>

                <p>
                    He believes that strong schools are built through meaningful relationships between students, educators, and families, and places great importance on open communication and community engagement.
                </p>

                <p>
                    Recognised with honours such as the{" "}
                    <strong className="text-[#831719]">
                        Times Education Icon Excellence in Education Leadership Award, Rabindranath Tagore National Awards, Young Change Agent in the Education Domain, and Teacher Trainer of the Year
                    </strong>,
                    Mr. Salunkhe is committed to creating a warm, values-driven school culture where children are encouraged to explore their potential, develop character, and grow into responsible global citizens.
                </p>

                <p>
                    His vision is to cultivate a safe, inclusive, and future-ready learning environment where every child is known, valued, and empowered to succeed.
                </p>
            </>
        ),
        quote:
            "To create a safe, inclusive, and future-ready learning environment where every child is known, valued, and empowered to succeed.",
    },
    {
        title: "Early Years Education Expert",
        name: "Aarvi Singh",
        img: "/images/leadership_team/aarvi.jpg",
        summary:
            "With over a decade of experience in early childhood education and academic leadership,..",
        content: (
            <>
                <p>
                    Ms. Aarvi Singh brings deep expertise in curriculum innovation, teacher training, and EdTech integration. Holding a Masters degree in Early Childhood Education, she has led the development of age-appropriate curriculum frameworks, creative learning materials, and assessment systems across a range of educational settings.
                </p>
                <p>
                    At Rising India School of Excellence, she plays a pivotal role in shaping integrated preschool programs, designing enrichment content, and driving professional development for educators and academic leaders. She has also been instrumental in establishing academic infrastructure and operational guidelines to ensure high-quality learning environments.
                </p>
                <p>
                    A strong advocate of digital learning, Ms. Singh has led initiatives such as home learning kits, chatbot-based learning flows, and blended curriculum models that combine technology with hands-on pedagogy. Her work spans private institutions, government programs, and early-years networks, consistently focusing on empowering educators and enhancing learner outcomes.
                </p>
            </>
        ),
        quote:
            "Education is not just about content—it’s about connection. When we empower teachers and engage learners with purpose, we create lifelong thinkers and compassionate leaders.",
    },
    // {
    //     title: "Administration and Admission Manager",
    //     name: "Aleena (Veena) Vikas Rane",
    //     img: "/images/leadership_team/aleena.jpg",
    //     summary:
    //         "With over a decade of experience in educational administration and institutional management,..",
    //     content: (
    //         <>
    //             <p>
    //                 Ms. Aleena Rane brings a strong blend of operational expertise, academic coordination, and people management skills. Her career spans reputed institutions including IIHM Goa, Bloomz International School, and CII Institute of Hospitality, where she has successfully led end-to-end administration, admissions, examinations, and student engagement functions.
    //             </p>
    //             <p>
    //                 A seasoned education professional, Ms. Rane has played a key role in establishing academic systems, ensuring compliance with regulatory bodies such as CBSE, IGNOU, and HRDF, and managing affiliations and audits with precision. Her leadership extends across staff recruitment, payroll management, budgeting, event coordination, and marketing initiatives, contributing to smooth institutional functioning and stakeholder satisfaction.
    //             </p>
    //             <p>
    //                 At Rising India School of Excellence, she spearheads the administration and enrolment operations, ensuring seamless processes, transparent communication, and an engaging experience for students and parents. Her proactive approach, team-building ability, and commitment to excellence make her a valued pillar of the school’s management.
    //             </p>
    //         </>
    //     ),
    //     quote:
    //         "Efficiency and empathy are the foundations of great administration — when systems run smoothly and people feel supported, learning truly thrives.",
    // },
    {
        title: "Deputy General Manager – Strategy & Planning",
        name: "Riya Jitendra Bohara",
        img: "/images/leadership_team/riya.jpg",
        summary:
            "With a strong foundation in business strategy and education management,..",
        content: (
            <>
                <p>
                    Ms. Riya Bohara brings a data-driven and analytical approach to strategic planning and institutional growth. Over the past two years, she has contributed to the design and execution of expansion strategies for the K–12 education segment, building on her prior experience in strategic planning for the preschool sector.
                </p>
                <p>
                    A graduate of the Indian Institute of Management (IIM) Raipur with a Master’s degree in Management and a Bachelor’s degree in Computer Engineering, Ms. Bohara combines technical insight with strategic foresight. Her expertise spans strategy, data modeling, and school expansion planning, enabling data-backed decisions that drive student and school growth.
                </p>
                <p>
                    At Rising India School of Excellence, she plays a key role in data-driven personalized learning strategies, measuring student outcomes and providing deep insights on progress of each student. At an organizational level, she steers growth initiatives, aligning academic and operational objectives.
                </p>
            </>
        ),
        quote:
            "Data tells a story - when we listen closely, it helps us make decisions that shape purposeful growth and lasting impact.",
    },
    {
        title: "Head of Centre of Excellence – Interior Design",
        name: "Sweta Gattani",
        img: "/images/leadership_team/sweta.jpg",
        summary: "With over 18 years of experience in Interior Design,..",
        content: (
            <>

                <p>
                    With over 18 years of experience in Interior Design, Sweta Gattani is an expert in designing impactful, experiential, and innovative learning spaces. Known for her expertise in spatial design, Sweta is well informed of emerging trends in 21st century learning zones, and she excels at combining functionality with aesthetics to create environments that are inspirational and practical.
                </p>
                <p>
                    A graduate in Interior Design from Rachana Sansad School of Interior Design and holding an MBA from top-tier University in Australia, Swetahas designed school & preschools where she has emphasized creating spaces that promote not only academic excellence but also ignite curiosity, creativity, collaboration and above all a sense of discovery and wonder for little learners.
                </p>
                <p>As someone who is ambitious, always open to new challenges and committed to continuous learning, she remains focussed on embracing the latest tools to deliver superior results for the Group.</p>
            </>
        ),
        quote:
            "The right design has the ability to transform a space into an inspiring environment that is future ready.",
    },
    // {
    //     title: "Brand Head – Rising India School of Excellence",
    //     name: "Sarvesha Mall",
    //     img: "/images/leadership_team/sarvesha.jpg",
    //     summary:
    //         "As a branding and marketing professional with over 12 years of corporate experience across diverse industries,..",
    //     content: (
    //         <>
    //             <p>
    //                 Including education, lifestyle, and automotive. Currently serving as the
    //                 <strong className="text-[#831719]"> Brand Head at Rising India School of Excellence</strong>
    //                 she has previously held leadership roles at
    //                 <strong className="text-[#831719]"> Aptech Ltd. </strong>
    //                 and
    //                 <strong className="text-[#831719]"> Audi India</strong>
    //                 , where she successfully drove marketing, brand strategy, and digital growth initiatives. A graduate in
    //                 <strong className="text-[#831719]"> Mass Communications and Videography</strong>
    //                 from
    //                 <strong className="text-[#831719]"> St. Xavier’s University</strong>
    //                 , with a
    //                 <strong className="text-[#831719]"> Diploma in Advertising</strong>
    //                 and a
    //                 <strong className="text-[#831719]"> Postgraduate qualification in Digital Marketing from MICA (upGrad)</strong>
    //                 , Sarvesha combines creative vision with strategic acumen. Her expertise spans
    //                 <strong className="text-[#831719]"> brand management, public relations, digital marketing, content strategy, and lifestyle consultancy</strong>
    //                 . Known for her people-centric approach and strong networking skills, she believes in the power of collaboration, innovation, and purpose-driven communication to build brands that make a lasting impact.
    //             </p>
    //         </>
    //     ),
    //     quote:
    //         "Driven by creativity with purpose - believes in transforming ideas into impactful brand experiences through innovation, teamwork, and consistency.",
    // },
    {
        title: "Human Resource Manager",
        name: "Jyotsna Hiwalkar",
        img: "/images/leadership_team/jyotsna.jpg",
        summary:
            "With over a decade of experience in human resources across education and corporate sectors,..",
        content: (
            <>
                <p>
                    I believe people are the true strength of any organization. At Rising India School of Excellence School, I focus on creating a positive, transparent, and growth-oriented work environment.
                </p>
                <p>
                    I have partnered with leaders and teams across organizations like Vishwajyot High School, Mindseed, Foundree,Enrich Salon and Qi Spine Clinic, leading initiatives in recruitment, performance management, employee engagement, and HR automation. Key achievements include ERP implementation and structured onboarding programs that align teams with organizational goals and strengthen workplace culture.
                </p>
                <p>I hold a Post Graduate Diploma in Human Resource Management from Welingkar Institute of Management.</p>
            </>
        ),
        quote:
            "Lead with empathy, nurture talent, and create an environment where every individual feels valued, motivated, and inspired to excel.",
    },
    {
        title: "Founder – White Canvas India",
        name: "Samaresh Shah",
        img: "/images/leadership_team/samaresh.jpg",
        summary:
            "A visionary leader and changemaker in the K–12 education space,..",
        content: (
            <>
                <p><strong className="text-[#831719]">Samaresh Shah</strong> is the Founder of <strong className="text-[#831719]">White Canvas India</strong>, dedicated to transforming education and empowering the next generation. With initiatives aligned to the <strong className="text-[#831719]">Prime Minister’s vision of Viksit Bharat @2047</strong>, he holds a <strong className="text-[#831719]">dual national record</strong> for achieving unprecedented admission facilitation footfall across India’s leading progressive schools.</p>
                <p>As <strong className="text-[#831719]">Honorary Mentor of Change</strong> with <strong className="text-[#831719]">Atal Innovation Mission (NITI Aayog)</strong> and <strong className="text-[#831719]">Immediate Co-Chair of PhD Chamber of Commerce and Industry</strong>, Samaresh has been instrumental in driving national entrepreneurship and innovation programs. He is the mind behind the globally recognized <strong className="text-[#831719]">Young CEO Program</strong>, the world’s largest entrepreneurial mindset initiative for teenagers, acknowledged even by the <strong className="text-[#831719]">Prime Minister’s Office</strong>.

                    An <strong className="text-[#831719]"> Electronics Engineer</strong> with executive education from <strong className="text-[#831719]">IIM Rohtak</strong>, he brings corporate leadership experience from <strong className="text-[#831719]">IBM, Capgemini, and Hewlett-Packard</strong> to the education sector.</p>
                <p>Recipient of <strong className="text-[#831719]">ET Now Leaders of Tomorrow, CNBC Thinkers of Tomorrow</strong> , and <strong className="text-[#831719]">BW Education STEM Solution of the Year</strong>, Samaresh also founded the <strong className="text-[#831719]">White Canvas Education Council</strong>, a collaborative platform for educators and policymakers shaping the future of learning.

                </p>
            </>
        ),
        quote:
            "Believes in creating meaningful impact through innovation, collaboration, and purpose-driven leadership, inspiring educators, entrepreneurs, and young changemakers nationwide.",
    },
    {
        title:
            (
                <>

                    <span className="block">Chairperson Emeritus, The Association of International Schools in India (TAISI)</span>
                    <span className="block"> Founder, Pioneering Schools</span>
                    <span className="block">Senior Education Consultant, Education Design International</span>
                </>
            )
        ,
        name: "Anu Monga (Advisor)",
        img: "/images/leadership_team/anu.jpg",
        summary: "Anu Monga is a distinguished educator,..",
        content: (
            <>
                <p>School leader, and entrepreneur with over 30 years of experience in international education. She has held significant roles, including Head of School at Bangalore International School and Director of Development at Kodaikanal International School. As Chairperson Emeritus of TAISI, she has been instrumental in fostering professional development and innovation across India's international schools. Anu is also a Senior Education Consultant at Education Design International, where she collaborates with school leaders to enhance teaching-learning outcomes and elevate curricula.</p>
            </>
        ),
        quote: `Anu's leadership philosophy centers on "heart-led leadership," emphasizing the importance of human connection, empathy, and joy in the learning process. She advocates for inclusive education that recognizes and celebrates diversity, ensuring that every child feels valued and supported. Her commitment to creating nurturing learning environments reflects her belief that education should be a transformative experience that empowers individuals and communities`,
    },
    {
        title: "Education Technology Leader",
        name: "Dr. Anuj Kacker",
        img: "/images/leadership_team/anuj.jpg",
        summary:
            "Dr. Anuj Kacker is a veteran of the education, training, and skill development sector with over three decades of leadership experience...",
        content: (
            <>
                <p>
                    Former CEO and Board Director of Aptech Ltd., he played a pivotal role in making Aptech a global leader, impacting over 100,000 students annually across 1,000+ centres in more than 35 countries.

                    Guided by his belief in long-term fulfilment of student aspirations, Dr. Kacker focuses on creating future-ready learners who are not only skilled but also well-rounded, confident individuals. He has directly engaged with tens of thousands of students, personal growth and value creation.
                </p>
                <p>
                    Dr. Kacker has actively contributed to national and industry education policy, including India’s AVGC Task Force, ASEAN-India and Indo-Africa Business Councils, and the Media & Entertainment Sector Skill Council. A strong advocate for AI in education, he continues to drive AI-enabled initiatives for learning, recruitment, and career advisory.

                    An alumnus of <strong className="text-[#831719]">IIT</strong> and <strong className="text-[#831719]">IIM Kolkata</strong> , he holds an <strong className="text-[#831719]">Honorary Doctorate from Lincoln University, Malaysia</strong> and remains a thought leader in education and the future of work, sharing insights on AI, learning, and skill development.
                </p>
            </>
        ),
        quote:
            "Dr. Kacker believes that education should empower every learner to be future-ready, fostering skills, confidence, and holistic growth to thrive personally and contribute meaningfully to society.",
    },
];


export const LeaderShipTeam = () => {
    return (
        <div>

            {teams.map(({ title, name, img, summary, content, quote }, id) => (
                <TeamCard title={title} name={name} img={img} summary={summary} content={content} quote={quote} key={id} />
            ))}


        </div>
    )
}